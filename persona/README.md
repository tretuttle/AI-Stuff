...
Multi-persona code review orchestrator for Claude Code. Get feedback from distinct expert perspectives -- each with their own philosophy, priorities, and blind spots.
...
Chat with Claude as any persona -- get their perspective with full tool access.
...
Unlike `/persona:review` (where personas are read-only subagents), `/persona:become` gives you the persona's voice and philosophy with full tool access -- they can read, write, edit, and run commands.
